...
// Thin adapter over the pure canvas-size rule so the rule itself stays
// testable without a whole scene.
TextCanvasSize resolveTextCanvasSize(const wpscene::WPTextObject& textObj) {
    return ResolveTextCanvasSize(textObj.size[0],
                                 textObj.size[1],
                                 textObj.maxwidth,
                                 textObj.pointsize,
                                 WPTextRenderer::kRasterDpiScale);
}
...
void registerObjectBindings(ParseContext& context, const nlohmann::json& json,
...
bool setHdrIntent(ParseContext& context, const wpscene::WPScene& sc, const nlohmann::json& json) {
...
std::shared_ptr<Scene> WPSceneParser::Parse(std::string_view scene_id, const std::string& buf,
    ...
    const bool effective_hdr = setHdrIntent(context, sc, json);
