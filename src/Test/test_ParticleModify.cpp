...
TEST_SUITE("ControlPointForce") {
    ...
    void applyControlPointForce(std::span<Particle> particles, const Eigen::Vector3d& cpOffset,
                                const Eigen::Vector3d& origin, float scale, float threshold,
                                double dt) {
    ...
    void applyControlPointAttract(std::span<Particle> particles, const Eigen::Vector3d& cpOffset,
                                  const Eigen::Vector3d& origin, float scale, float threshold,
                                  double dt) {
        Eigen::Vector3d offset = cpOffset + origin;
        for (auto& p : particles) {
