...
TEST_SUITE("Bswap") {
    ...
    TEST_CASE("uint8_t identity") { CHECK(bswap<uint8_t>(0xAB) == 0xAB); }
...
TEST_SUITE("ArrayHelper") {
    ...
    TEST_CASE("transform doubles values") {
        ...
        auto             result = transform<int>(std::span<const int>(src), [](int x) {
            return x * 2;
        });
        CHECK(result.size() == 3);
        CHECK(result[0] == 2);
...
TEST_SUITE("past_last_slash") {
    ...
    TEST_CASE("empty string returns empty") { CHECK(std::strcmp(forceRuntime(""), "") == 0); }
...
std::set<std::string> scanRegisterAsset(const std::string& src) {
    static const std::regex re(R"(registerAsset\s*\(\s*['"]([^'"]+)['"]\s*\))");
    std::set<std::string>   out;
    for (auto it = std::sregex_iterator(src.begin(), src.end(), re); it != std::sregex_iterator();
         ++it) {
        out.insert((*it)[1].str());
    }
    return out;
...
TEST_SUITE("asset pre-scan regex") {
    ...
    TEST_CASE("registerAsset — multiple calls all captured") {
        auto s = scanRegisterAsset("var a = engine.registerAsset('x.json');\n"
    ...
    TEST_CASE("createLayer literal — single-line with image key first") {
        auto s =
            scanCreateLayerLiteral(R"(thisScene.createLayer({image: 'trail.json', alpha: 0});)");
    ...
    TEST_CASE("createLayer literal — double-quoted image with single-quoted path") {
        auto s = scanCreateLayerLiteral(R"(thisScene.createLayer({"image": 'mixed.json'}))");
    ...
    TEST_CASE("createLayer literal — ignore call with no image key") {
        ...
        auto s = scanCreateLayerLiteral(R"(var l = thisScene.createLayer(myAsset);)");
        CHECK(s.empty());
    }

    TEST_CASE("createLayer literal — repeated paths collapse to unique set") {
        auto s = scanCreateLayerLiteral("thisScene.createLayer({image: 'a.json'});\n"
    ...
    TEST_CASE("pool-size hint — no pool pattern returns 0 (stay on default)") {
        ...
        CHECK(deriveAssetPoolHint("thisScene.createLayer({image: 'coin.json'});\n"
                                  ".addSlider({name: 'trailLength', max: 400, integer: true});") ==
              0);
    }

    TEST_CASE("pool-size hint — trailPointsPool.pop triggers larger pool") {
        // 3body MAIN: own pool + trailLength slider.  Hint = 400 × 3 = 1200.
        std::string src = "let trailPointsPool = [];\n"
                          "function get() {\n"
...
static bool load(const std::string& path, Pkg& p) {
    ...
    for (uint32_t i = 0; i < count; i++) {
        Entry e;
        if (! read_sized_string(f, e.path) || ! read_u32_le(f, e.off) || ! read_u32_le(f, e.size))
            return false;
...
TEST_SUITE("wp-pkg pack format") {
    ...
    TEST_CASE("Pack skips .info.txt sidecars extract --raw=off writes") {
        ...
        {
            std::ofstream f(dir + "/real.tex", std::ios::binary);
            f << "Xyz";
        }
        {
            std::ofstream f(dir + "/real.tex.info.txt", std::ios::binary);
            f << "summary";
        }
        {
            std::ofstream f(dir + "/scene.json.orig", std::ios::binary);
            f << "{}";
        }
        {
