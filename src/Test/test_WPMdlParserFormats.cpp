...
TEST_CASE("flag 11: pos + normal + texcoord") {
    ...
    for (int i = 0; i < 2; i++) {
        b.f32(i); b.f32(0); b.f32(0);            // pos
        b.f32(0); b.f32(1); b.f32(0);            // normal
        b.f32(0.1f * i); b.f32(0.2f * i);        // uv
    }
    b.u32(0); // no indices
...
TEST_CASE("multiple submeshes parse independently") {
    ...
    // submesh 1
    b.str("b.json"); b.i32(0); b.u32(32);
    b.f32(9); b.f32(0); b.f32(0);
...
TEST_CASE("bone size != 64 aborts parse") {
    ...
    b.i32(0); b.i32(1); b.i32(1);
    b.str(""); b.i32(0);
    b.u32(kStdHerald); b.u32(0); b.u32(0);
    appendMdls(b, 1);
    b.u32(0);
    b.u16(1); b.u16(0);
    b.str("b");
...
TEST_CASE("puppet with negative animation id fails") {
    ...
    b.u8(0xFF); b.u8(0xFF); b.u8(0xFF); b.u8(0xFF);
    fs::MemBinaryStream f(takeBuffer(std::move(b)));
    WPMdl               mdl;
...
TEST_CASE("puppet with vertices + indices + MDLS2 extras") {
    ...
    b.f32(0.5f); b.f32(-0.25f); b.f32(1.0f);   // pos
    b.u32(0); b.u32(1); b.u32(2); b.u32(3);    // blend indices
    b.f32(0.25f); b.f32(0.25f); b.f32(0.25f); b.f32(0.25f); // weights
    b.f32(0.1f); b.f32(0.9f);                   // uv
    // one triangle = 6 bytes
    b.u32(6);
    b.u16(0); b.u16(0); b.u16(0);

    appendMdls(b, 2); // MDLS v2 — triggers mdls > 1 extras block
    b.u32(0); b.u16(0); b.u16(0); // bones_file_end, bones_num=0, unk
...
TEST_CASE("puppet with MDLS2 extras + downstream MDAT catches skip-arithmetic mutants") {
    ...
    b.u32(kStdHerald); b.u32(0); b.u32(0);
    appendMdls(b, 2);
    b.u32(0); b.u16(2); b.u16(0); // 2 bones
...
TEST_CASE("animation with three events stores all three distinctly") {
    ...
    b.str("MDLA0001");
    b.u32(0); b.u32(1);
    b.u32(100); b.i32(0); b.str("walk"); b.str("loop");
    b.f32(30); b.i32(60); b.i32(0);
    b.u32(1);
    b.i32(0); b.u32(36);
    for (int i = 0; i < 9; i++) b.f32(0);
    ...
    for (int i = 0; i < 3; i++) {
        b.f32(0.1f * i);
        std::string js =
            std::string(R"({"frame":)") + std::to_string(frames[i])
            + R"(,"name":")" + ns[i] + R"("})";
        b.str(js);
    }
...
TEST_CASE("animation with two bframes stores both") {
    ...
    b.str("MDLA0001");
    b.u32(0); b.u32(1);
    b.u32(42); b.i32(0); b.str("a"); b.str("loop");
    b.f32(30); b.i32(10); b.i32(0);
...
TEST_CASE("animation playmode mirror and single both decode") {
    auto makeWith = [](const char* mode) {
        ...
        b.u32(0); b.u16(0); b.u16(0);
        b.str("MDLA0001");
        b.u32(0); b.u32(1); // end_size, anim_num
        b.u32(7);           // id
        b.i32(0); b.str("anim"); b.str(mode);
        b.f32(30); b.i32(1); b.i32(0);
        b.u32(1);
        b.i32(0); b.u32(36);
        for (int i = 0; i < 9; i++) b.f32(0);
        b.u32(0); // no events
