...
TEST_CASE("default-constructed mesh: no arrays, no material, TRIANGLE primitive, point size 1") {
    SceneMesh m;
...
TEST_CASE("AddVertexArray/AddIndexArray grow the internal vectors") {
    SceneMesh    m;
    std::vector<Attr> attrs { Attr { .name = "pos", .type = VertexType::FLOAT3, .padding = false } };
...
TEST_CASE("GetVertexArray/GetIndexArray return reference to stored data") {
    SceneMesh    m;
    std::vector<Attr> attrs { Attr { .name = "p", .type = VertexType::FLOAT1, .padding = false } };
